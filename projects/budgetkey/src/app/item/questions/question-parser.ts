...
export class QuestionParser {
    ...
    private static getFormatter(mod: string, themeId: string) {
      ...
      if (mod === 'number') {
        return (x: any, row: any) => {
          ...
          return isFinite(x)
            ? QuestionParser.format.number(x)
            : '-';
        };
        ...
        if (mod === 'item_link') {
          ...
          return (x: any, row: any) => {
            if (themeId) {
              return '<a href="/s/?q=' + encodeURIComponent(x) + '&theme=' + themeId + '" title="חיפוש מידע נוסף על  ' + x + '">' + x + '</a>';
            } else {
    ...
    static processQuestions(questions: Question[], themeId: string) {
