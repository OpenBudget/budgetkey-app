...
import { __T as __ } from '../main-page.component';
...
class TransitionStep implements Mutator {
  ...
  constructor(private _mutators: Array<Mutator>) {
    this._duration = d3Max(_mutators, (m) => m.duration()) || 0;
  }
...
class TransitionSteps extends BaseMutator {
  constructor(private _steps: Array<TransitionStep>) {
    super();
    const totalLength = d3Sum(_steps, (s) => s.duration());
    let ofs = 0;
  ...
  override mutate(hero: HeroComponent, t: number): void {
...
class ChartIntroduce extends BaseMutator {
  ...
  override mutate(hero: HeroComponent, t: number): void {
    hero.chartOpacity = easeExpOut(this.increasing ? t : 1 - t);
  }
}
...
function initDialog() {
  const dialog = [
    ...
    A(__('&hellip;')),
    A(__('הופ!')),
    A(__('תקציב משרד החינוך ב-2024')),
    A(__('מעל 80 מיליארד שקלים')),
...
export class HeroComponent implements AfterViewInit {
  ...
  makeDeficitCharts(data: any) {
    ...
    const expenseChildren: any[] = data.expenseChildren.sort((a: any, b: any) => { return b.net_revised - a.net_revised; });
    const semiExpenseRest = data.budget;
    const expenseRest = semiExpenseRest - d3Sum(
      expenseChildren.slice(1, 4).map(
    ...
    const incomeChildren: Array<any> = data.incomeChildren.sort((a: any, b: any) => { return b.net_allocated - a.net_allocated; });
    const kidsNum = [1, 2, 3, 4];

    const expandedIncomesFlowGroup = kidsNum.map((k) => {
      const incomeRest = data.income - d3Sum(
        incomeChildren.slice(0, k).map(
          (d: any) => d.net_revised || d.net_allocated)
      );
      const flows = [];
      for (let i = 0 ; i < k ; i++ ) {
  ...
  makeEducationCharts(data: any) {
    const numToShow = 3;
    const children: Array<any> = data.children.sort((a: any, b: any) => { return (b.net_revised || b.net_allocated) - (a.net_revised || a.net_allocated); });
  ...
  makeSupportChart(supportData: any, budgetData: any) {
    const numToShow = 5;
    const children: Array<any> = supportData.sort((a: any, b: any) => { return b.total_amount - a.total_amount; });
    const total = d3Sum(children.map((d) => d.total_amount));
    const rest = total - d3Sum(
      children.slice(0, numToShow).map(
